...
func newTestServerFull(t *testing.T, resolver Resolver, authr Authenticator, limiter RateLimiter) *httptest.Server {
	...
	srv := httptest.NewServer(NewRouter(resolver, authr, limiter, stubBudgetTracker{}, stubCostCalculator{}, stubHealthRecorder{}, nil, nil, nil, noRetryConfig(t), nil, nil, discardLogger(), func() bool { return true }))
	t.Cleanup(srv.Close)
	return srv
}
...
func TestProbes(t *testing.T) {
	t.Run("healthz is always ok", func(t *testing.T) {
		srv := httptest.NewServer(NewRouter(stubResolver{}, stubAuthenticator{}, stubRateLimiter{}, stubBudgetTracker{}, stubCostCalculator{}, stubHealthRecorder{}, nil, nil, nil, noRetryConfig(t), nil, nil, discardLogger(), func() bool { return false }))
		defer srv.Close()
	...
	t.Run("readyz reflects readiness", func(t *testing.T) {
		...
		for ready, want := range tests {
			srv := httptest.NewServer(NewRouter(stubResolver{}, stubAuthenticator{}, stubRateLimiter{}, stubBudgetTracker{}, stubCostCalculator{}, stubHealthRecorder{}, nil, nil, nil, noRetryConfig(t), nil, nil, discardLogger(), func() bool { return ready }))

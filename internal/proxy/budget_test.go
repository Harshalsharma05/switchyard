...
func newTestServerWithBudget(t *testing.T, resolver Resolver, authr Authenticator, limiter RateLimiter, budgetTracker BudgetTracker, calc CostCalculator) *httptest.Server {
	t.Helper()
	srv := httptest.NewServer(NewRouter(resolver, authr, limiter, budgetTracker, calc, stubHealthRecorder{}, nil, nil, nil, noRetryConfig(t), nil, nil, discardLogger(), func() bool { return true }))
	t.Cleanup(srv.Close)
	return srv
}

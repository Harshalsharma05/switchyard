...
func TestMonitorStatusSurvivesRestart(t *testing.T) {
	...
	first, err := NewMonitor(providers, NewRecorder(providers), nil, rdb, nil, log, cfg)
	if err != nil {
	...
	second, err := NewMonitor(providers, NewRecorder(providers), nil, rdb, nil, log, cfg)
	if err != nil {
		t.Fatalf("NewMonitor() error: %v", err)
	}
...
func TestMonitorRestoreIsFailOpenOnUnknownKey(t *testing.T) {
	...
	m, err := NewMonitor(providers, NewRecorder(providers), nil, rdb, nil, log, testMonitorConfig())
	if err != nil {
		t.Fatalf("NewMonitor() error: %v", err)
	}
